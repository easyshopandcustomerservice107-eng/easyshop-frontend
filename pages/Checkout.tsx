...
const CheckoutForm: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-white pt-10 pb-4 md:pt-16 md:pb-8 font-sans text-black">
        ...
        <div className="flex items-center justify-center mb-10 max-w-2xl mx-auto">
          <div className="flex items-center w-full">
            <div className="flex flex-col items-center">
              <div className="w-8 h-8 rounded-full bg-accent-gold text-primary flex items-center justify-center text-xs font-black shadow-lg">1</div>
              <span className="text-[11px] font-black mt-2 text-accent-gold">Cart</span>
            </div>
            <div className="flex-1 h-[1px] bg-accent-gold mx-4 -mt-6" />
            <div className="flex flex-col items-center">
              <div className="w-8 h-8 rounded-full bg-accent-gold text-primary flex items-center justify-center text-xs font-black shadow-lg">2</div>
              <span className="text-[11px] font-black mt-2 text-accent-gold">Shipping</span>
            </div>
            <div className="flex-1 h-[1px] bg-black/10 mx-4 -mt-6" />
            <div className="flex flex-col items-center opacity-40">
              <div className="w-8 h-8 rounded-full bg-white border border-black/20 text-black/20 flex items-center justify-center text-xs font-black">3</div>
              <span className="text-[11px] font-black mt-2">Payment</span>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
          ...
          <div className="lg:col-span-8 space-y-6">
            ...
            {!clientSecret && !orderReadyForPaypal ? (
                    ...
                    <div className="space-y-6">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <input
                          ...
                          className="w-full bg-white border border-black/10 p-4 rounded-sm focus:border-accent-gold outline-none text-sm font-medium"
                        />
                        <input
                          required
                          type="tel"
                      ...
                      <textarea
                        ...
                        className="w-full bg-white border border-black/10 p-4 rounded-sm focus:border-accent-gold outline-none text-sm h-24 resize-none font-medium"
                      />
                    </div>
                  )}
                </div>
                          ...
                          <div className="flex-1">
                            ...
                            <div className="flex items-center justify-between">
                              ...
                              <div className="text-right">
                                <p className="text-base font-bold text-[#f85606]">PKR {(item.price * item.quantity).toFixed(2)}</p>
                                {item.product?.discount > 0 && (
                                  <p className="text-[10px] text-gray-400 line-through decoration-rose-400/20">PKR {(item.product.price * item.quantity).toFixed(2)}</p>
                                )}
                              </div>
                            </div>
                          </div>
                  ...
                  <h3 className="text-base font-bold text-gray-800 uppercase mb-6 tracking-tight">Select Payment Method</h3>
                    ...
                    {gateways.filter(gw => gw.gateway !== 'stripe').map(gw => (
                      <button
                        ...
                        className={`p-4 border rounded-sm text-center transition-all relative group flex flex-col items-center justify-center gap-3 hover:bg-black/5 ${selectedGateway === gw.gateway ? 'border-accent-gold bg-accent-gold/5' : 'border-black/10'}`}
                      >
                        {selectedGateway === gw.gateway && (
                          <div className="absolute top-2 right-2 text-accent-gold">
                            <CheckCircle2 className="h-4 w-4" />
                          </div>
                        )}
                        <div className={`w-12 h-12 rounded-sm flex items-center justify-center transition-all ${selectedGateway === gw.gateway ? 'text-accent-gold' : 'text-black/20'}`}>
                          {gw.gateway === 'paypal' ? (
              ...
              <div className="bg-white rounded-sm p-8 shadow-sm border border-black/5 animate-in fade-in slide-in-from-bottom-4">
                <div className="flex items-center gap-3 mb-8">
                  <div className="w-10 h-10 rounded-full bg-accent-gold text-primary flex items-center justify-center text-lg font-black shadow-lg">3</div>
                  <h3 className="text-xl font-black text-black uppercase tracking-tighter">Secure Payment</h3>
                </div>
            ...
            <div className="bg-white rounded-lg p-6 shadow-sm border border-gray-100">
              ...
              <div className="space-y-4 text-sm mb-6">
                <div className="flex justify-between items-center">
                  ...
                  <span className="font-bold text-gray-800">PKR {subtotal.toFixed(2)}</span>
                </div>
                <div className="flex justify-between items-center">
              ...
              <div className="pt-6 border-t border-gray-100 mb-8">
                <div className="flex justify-between items-center mb-1">
                  ...
                  <span className="text-2xl font-bold text-[#f85606] tracking-tight">PKR {total.toFixed(2)}</span>
                </div>
              ...
              {!clientSecret && !orderReadyForPaypal && (
                <button
                  ...
                  className="w-full bg-accent-gold text-primary py-5 rounded-sm font-black text-sm uppercase tracking-[0.4em] hover:bg-black hover:text-white transition-all shadow-xl disabled:opacity-50 flex items-center justify-center gap-4"
                >
                  {isProcessing ? <Loader size="sm" color="white" /> : 'Complete Order'}
                </button>
              )}
